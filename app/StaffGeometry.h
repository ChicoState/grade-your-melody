...
// Single source of truth for staff layout constants shared between
// StaffAreaWidget (which draws the lines) and StaffWidget (which places notes).
struct StaffGeometry {
    ...
    // Y coordinate of the top staff line (row 10 = F5).
    static constexpr int staffTopY() { return marginY; }

    // Pixel distance between the top and bottom staff lines for a given cellH.
    static constexpr int staffSpanPx(int cellH) {
        return (lineRows[4] - lineRows[0]) * cellH; // (10-2)*cellH = 8*cellH
    }

    // Y coordinate of the bottom staff line (row 2 = E4).
